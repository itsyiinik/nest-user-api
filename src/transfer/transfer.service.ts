...
export class TransferService {
  ...
  async transferBalance(fromUserId: string, dto: TransferBalanceDto) {
    ...
    const fromUser = await this.userService.findUserWithBalance(fromUserId);
    if (!fromUser) {
    ...
    const toUser = await this.userService.findUserByExactLogin(dto.toUserLogin);
    if (!toUser) {
    ...
    await this.userService.updateBalance(fromUser.id, newFromBalance);
    await this.userService.updateBalance(toUser.id, newToBalance);
